...
function extractYouTubeKey(href: string): string | null {
  try {
...
function TrailerButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="inline-flex h-[clamp(52px,6vh,68px)] items-center gap-[clamp(0.75rem,1vw,1rem)] rounded-[18px] bg-[var(--color-accent)] px-[clamp(1.4rem,2.2vw,2.5rem)] font-[family-name:var(--font-heading)] text-[clamp(0.95rem,1vw,1.05rem)] font-semibold text-[var(--color-accent-contrast)] transition hover:brightness-105"
    >
...
export function HomeLandingHero({
  ...
  const handleTrailerClick = useCallback((slide: HomepageHeroSlide) => {
    const key = slide.item.trailerKey || extractYouTubeKey(slide.item.trailerHref);
    if (key) {
  ...
  const handleBackdropClick = useCallback((slide: HomepageHeroSlide) => {
    if (slide.item.watchHref) {
      router.push(slide.item.watchHref);
    }
  }, [router]);
      ...
      <section className="relative min-h-[calc(100dvh+80px)] overflow-hidden">
        <AnimatePresence mode="wait">
          <motion.div
            ...
            onClick={() => handleBackdropClick(slide)}
          >
            <Image
              src={heroImage}
